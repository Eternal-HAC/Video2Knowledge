...
- `app/whisper.py`: deterministic Mock local Whisper backend plus `FasterWhisperBackend` for lazily loading the optional dependency and mapping an existing `NormalizedAudio` into `TranscriptResult`. `FasterWhisperBackend` can be injected into the non-CLI `transcribe_local_media` boundary, but the default import pipeline and `real-fallback` do not select it automatically.
