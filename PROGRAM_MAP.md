...
- `app/transcript.py`: transcript provider boundary with Mock implementation, YouTube official VTT/WebVTT subtitles, fallback eligibility policy, and real fallback orchestration into the Mock Whisper backend. Does not use automatic captions, transcript API fallback, audio acquisition, or real Whisper execution.
